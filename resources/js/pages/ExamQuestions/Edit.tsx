...
export default function Edit({ examQuestion, assignments, clos, committees }: any) {
    const { data, setData, put, processing, errors } = useForm({
        course_id: examQuestion.course_id?.toString() || '',
        session: examQuestion.session || '',
        semester: examQuestion.semester || '',
        total_marks: examQuestion.total_marks || 70,
        duration: examQuestion.duration || '3 Hours',
        moderation_committee_id: examQuestion.moderation_committee_id?.toString() || '',
        items: examQuestion.items?.map((item: any) => ({
            id: item.id,
            question_label: item.question_label,
            question_text: item.question_text,
            marks: item.marks,
            clo_id: item.clo_id?.toString() || '',
            blooms_taxonomy_level: item.blooms_taxonomy_level || 'Remember',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('exam-questions.update', examQuestion.id));
    };

    const handleSubmitForModeration = () => {
        if (!data.moderation_committee_id) {
            alert('Please select a moderation committee before submitting.');
            return;
        }
        if (confirm('Are you sure you want to submit this question for moderation? You cannot edit it afterwards until revision is requested.')) {
            router.post(route('exam-questions.submit', examQuestion.id));
        }
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <h2 className="text-2xl font-bold tracking-tight">Edit Exam Question Paper</h2>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <Badge variant={examQuestion.status === 'RevisionNeeded' ? 'destructive' : 'outline'}>
                            {examQuestion.status}
                        </Badge>
                    </div>
                </div>
                ...
                {examQuestion.status === 'RevisionNeeded' && examQuestion.moderator_feedback && (
                ...
                <form onSubmit={submit} className="space-y-6">
                        ...
                        <CardContent className="grid gap-6">
                            <div className="grid gap-6 md:grid-cols-2">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        value={`${examQuestion.course?.code} - ${examQuestion.course?.name}`}
                                        disabled
                                        className="bg-muted"
                                    />
                    ...
                    <div className="space-y-4">
                        ...
                        {data.items.map((item, index) => (
                            <Card key={index} className="relative border-l-4 border-l-blue-500 hover:shadow-md transition-shadow">
                    ...
                    <div className="flex justify-between gap-4 pt-4 border-t">
                        ...
                        <div className="flex gap-4">
                            ...
                            <Button
                                ...
                                disabled={processing || !data.moderation_committee_id}
                            >
